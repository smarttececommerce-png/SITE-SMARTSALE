...
const getGlobalData = () => ({
    ...
    userMetas: JSON.parse(JSON.stringify(allUserMetas)),
});
...
function initializeDayjs() {
    try {
        const plugins = ['customParseFormat', 'utc', 'timezone', 'isSameOrAfter', 'isSameOrBefore', 'isBetween'];
        plugins.forEach(p => {
...
function loadAllRealtimeData() {
    ...
    onSnapshot(collection(db, "userMetas"), (snapshot) => {
        allUserMetas = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        window.dispatchEvent(new CustomEvent('dataUpdated', { detail: { dataType: 'userMetas' } }));
    });
...
async function updateDashboardView() {
    ...
    if (workingNowList && workingNowCount && window.dayjs) {
        
        const today = dayjs().format('YYYY-MM-DD');
        
        const todaysActiveRecords = allPontoRecords.filter(r => 
            r.id && r.id.endsWith(today) && r.status === 'em_andamento'
        );
        
        const usersMap = new Map(allUsers.map(user => [user.id, user]));
        const workingUsersWithTime = todaysActiveRecords.map(record => ({
            user: usersMap.get(record.employeeId),
            record: record
        })).filter(item => item.user);

        workingNowCount.textContent = workingUsersWithTime.length;
        workingNowList.innerHTML = workingUsersWithTime.length > 0
            ? workingUsersWithTime.map(item => {
                const clockInTime = dayjs(item.record.entrada).format('HH:mm');
                return `
                    <div class="summary-item">
                        <span>${item.user.nomeFantasia}</span>
                        <span class="text-gray-400 text-sm">Entrada: ${clockInTime}</span>
                    </div>
                `;
            }).join('')
            : '<p class="text-gray-400 text-sm">Nenhum funcionário com ponto ativo.</p>';
    }
    ...
    if (completedTasksTodayCount && window.dayjs) {
        const today = dayjs().format('YYYY-MM-DD');
...
window.db = db;
// CORREÇÃO: Remove o hífen inválido.
